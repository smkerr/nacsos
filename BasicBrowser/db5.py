...
def add_term(word):
    term = Term(title=word,run_id=run_id)  
    term.save()   
...
def first_word():
    f = Term.objects.filter(run_id=run_id).first()
    return(f.term)

def first_topic():
    f = Topic.objects.filter(run_id=run_id).first()
    return(f.topic)
...
def add_doc_simple(UT,d):
    doc = Doc(UT=UT,modkey=d)
    doc.save()

def update_doc_simple(d):
...
def add_doc_topic_dtm(doc_id, topic_id, score, scaled_score):
    doc = Doc.objects.get(modkey=doc_id)
    topic = Topic.objects.get(topic=topic_id)
    dt = DocTopic(doc=doc, topic=topic, score=score, scaled_score=scaled_score,run_id=run_id)
    dt.save()
