...
function heatmap_display(url, heatmapId, paletteName) {
    ...
    var lOffset = (viewerWidth - legendElementWidth * classesNumber) / 2 - viewerPosLeft

    // http://bl.ocks.org/mbostock/5577023
    var colors = colorbrewer[paletteName][classesNumber].reverse();
    ...
    d3.json(url, function(error, data) {
        ...
        var m = d3.max(d3.max(data.data))
        //console.log(col_number, row_number);

        var viewerHeight = row_number * cellSize + 400

        var lMargin = (viewerWidth - cellWidth * col_number)/2 - viewerPosLeft
        ...
        var colLabels = svg.append("g")
            ...
            .attr("x", 0)
            .attr("y", function(d, i) {
                return (lMargin + i * cellWidth);
            })
            .style("text-anchor", "left")
            .attr("transform", function(d, i) {
                return "translate(" + cellWidth / 2 + ", -3) rotate(-90) rotate(45, 0, " + (lMargin + i * cellWidth) + ")";
            })
        ...
        legend.append("svg:rect")
            .attr("x", function(d, i) {
                return lOffset + legendElementWidth * i;
            })
            .attr("y", viewerPosTop)
            ...
            if (rORc == "r") { // sort on cols
                ...
                t.selectAll(".cell")
                    .attr("x", function(d) {
                        ...
                        return lMargin + sorted.indexOf(col) * cellWidth;
                    });
                t.selectAll(".colLabel")
                    .attr("y", function(d, i) {
                        return lMargin + sorted.indexOf(i) * cellWidth;
                    })
                    .attr("transform", function(d, i) {
                        //return "translate(" + cellWidth / 2 + ", -3) rotate(-90) rotate(45, 0, " + (lMargin + i * cellWidth) + ")";
                        return "translate(" + cellWidth / 2 + ", -3) rotate(-90) rotate(45, 0, " + (lMargin + sorted.indexOf(i) * cellWidth) + ")";
                    });
            } else { // sort on rows
	    ...
	    var newOrder = d3.select("#order").property("value");
            changeOrder(newOrder, heatmapId, lMargin);
        });
...
//#########################################################
function changeOrder(newOrder, heatmapId, lMargin) {
